...
export const BackgroundBoxes = ({ className }: { className?: string }) => {
  ...
  return (
    ...
    >
      {rows.map((_, i) => (
        <motion.div key={`row` + i} className="w-16 h-8 border-l border-slate-300/25 dark:border-[#ec1e24]/15 relative transition-colors duration-300">
          {cols.map((_, j) => (
            <motion.div
              whileHover={{
              ...
              key={`col` + j}
              className="w-16 h-8 border-r border-t border-slate-300/25 dark:border-[#ec1e24]/15 relative cursor-pointer transition-colors duration-300"
            >
            </motion.div>
          ))}
        </motion.div>
